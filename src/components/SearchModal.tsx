...
export const SearchModal: React.FC<SearchModalProps> = React.memo(({
  ...
  const results = useMemo(() => {
    ...
    for (let i = 0; i < flatNodes.length; i++) {
      ...
      if (node) {
        const fullPath = getFullPath(node.id, flatNodes);
        if ((node.name && node.name.toLowerCase().includes(q)) || (fullPath && fullPath.toLowerCase().includes(q))) {
          matches.push(node);
  ...
  return (
    <div className='fixed inset-0 z-50 flex items-start justify-center pt-16 sm:pt-24 px-4 bg-background/80 backdrop-blur-md animate-in fade-in duration-150'>
      <div className='w-full max-w-2xl bg-surface border border-surface-border rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[80vh]'>
        ...
        {/* Results List */}
        <div className='flex-1 overflow-y-auto p-2 space-y-1.5 scrollbar-thin scrollbar-thumb-surface-border'>
          {query.trim() === "" ? (
            ...
            <div className='p-8 text-center text-slate-500 text-xs'>No matching files or folders found for &quot;{query}&quot;</div>
          ) : (
              ...
              return (
                <Card
                  ...
                  className={`rounded-lg! flex items-center gap-3 p-2.5 transition-colors cursor-pointer text-xs border ${
                    isSelected
                      ? "bg-surface-hover! text-slate-100! border-slate-600! shadow-sm"
                      : "bg-surface/50! text-slate-400! border-surface-border! hover:bg-surface-hover! hover:text-slate-200! hover:border-slate-600!"
                  }`}
                >
                  ...
                  <div className='flex-1 min-w-0'>
                    ...
                    <p className='text-[11px] font-mono text-slate-400 truncate mt-0.5' title={getFullPath(node.id, flatNodes)}>
                      {truncate(getFullPath(node.id, flatNodes), 60)}
                    </p>
                  </div>
        ...
        <div className='px-4 py-2 bg-background/60 border-t border-surface-border flex items-center justify-between text-[11px] text-slate-400'>
          <span>{results.length > 0 ? `Showing ${results.length} matches` : "Ready"}</span>
